    ...
    for i in range(num_detections):
        if scores_[i] > confidence:
            ...
            if len(x_vect) > 0:
                ...
                width_object = calcul_taille(distance, xmin*width, xmax*width)
                height_object = calcul_taille(distance, ymin*height, ymax*height)
                ...
                draw.text((xmin*width, ymin*height), 'Largeur : ' + str(width_object) , fill=(255,0,0), font=font)
                draw.text((xmin*width, ymin*height+20), 'Hauteur : ' + str(height_object), fill='red', font=font)
...
def calcul_taille(distance, pos_min, pos_max):

    adjacent = distance/math.tan(math.radians(47.5))
    taille_fond = 2*adjacent

    taille_pixel = taille_fond/width

    taille_objet_pixel = (pos_max - pos_min)*taille_pixel

    return taille_objet_pixel
        ...
        with tf.Session(config=config, graph=detection_graph) as sess:
            while not exit_signal:
                ...
                if new_data:
                    ...
                    cv2.imshow('ZED depth map', cv2.resize(depth_np, (width, height)))
